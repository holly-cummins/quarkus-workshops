...
To check that the `smallrye-health` extension is working as expected, make sure the Hero service is up and running (`./mvnw quarkus:dev`) and access using your browser or cURL the following URLs:
...
image::observability-health-ui.png[role=half-size]
...
[source,java]
----
include::{projectdir}/rest-heroes/src/main/java/io/quarkus/workshop/superheroes/hero/health/PingHeroResourceHealthCheck.java[]
----
====
