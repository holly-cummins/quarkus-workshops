...
That should give a failure.
This is good, because it tells us our tests are working, and can guide the implementation.
...
This test failure tests us that although the implementation doesn't do exactly what we want, it's doing something.
As well as the tests, the scaffolding gave us a basic implementation.
